...
export default function Dashboard() {
  ...
  const enterDepartment = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await axios.post('http://localhost:5000/department-data', { department: currentDept, password: deptPassword }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const sendRequest = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('token');
    try {
        await axios.post('http://localhost:5000/request-access', reqData, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await axios.post('http://localhost:5000/upload', { ...uploadData, department: currentDept }, { headers: { Authorization: `Bearer ${token}` } });
      closeModal();
      const res = await axios.post('http://localhost:5000/department-data', { department: currentDept, password: deptPassword }, { headers: { Authorization: `Bearer ${token}` } });
      setFiles(res.data.files);
      fetchStats();
      showToast("Secure Upload Complete", 'success');
    } catch (err) { showToast("Upload Rejected", 'error'); }
    finally { setLoading(false); }
  };
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-4 w-full md:w-auto bg-white/5 border border-white/10 p-2 rounded-3xl backdrop-blur-md shadow-xl">
            <div className="relative flex-1 md:w-64">
                ...
                <input 
                    ...
                    placeholder={view === 'hub' ? "Filter Departments..." : "Search Files..."}
                    className="w-full bg-transparent border-none outline-none text-sm text-white pl-11 pr-4 placeholder-slate-500"
                />
                                ...
                                <GlassCard key={dept} delay={i * 0.05} onClick={() => { setCurrentDept(dept); setDeptPassword(''); setModals({...modals, auth: true}) }} className="group cursor-pointer hover:border-violet-500/30">
                                    <div className="p-8">
                                        <div className="flex justify-between items-start mb-8">
                                            <div className={`p-4 rounded-2xl ${isUnlocked ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/10 text-slate-400 group-hover:text-white'} transition-colors`}>
                                                <Briefcase size={28}/>
                                            </div>
                                            <Badge color={isUnlocked ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300' : 'bg-slate-500/10 border-slate-500/20 text-slate-400'}>
            ...
            {view === 'department' && (
                <motion.div key="dept" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} transition={{ duration: 0.3 }}>
                    <div className="flex justify-between items-center mb-8">
                        <button onClick={() => setView('hub')} className="flex items-center gap-2 text-slate-400 hover:text-white transition-colors px-4 py-2 rounded-xl hover:bg-white/5">
                            <ChevronRight className="rotate-180" size={20}/> Back to Hub
                        </button>
                        <button onClick={() => setModals({...modals, upload: true})} className="bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-violet-500/20 flex items-center gap-2 transition-all active:scale-95">
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-2xl">
                        <div className="flex items-end justify-between mb-8 pb-6 border-b border-white/10">
                            <div>
                                <h2 className="text-4xl font-bold text-white mb-2">{currentDept}</h2>
                                <p className="text-slate-400 flex items-center gap-2">
                                    <FolderOpen size={16} className="text-violet-400"/> / Root / {currentDept} / <span className="text-violet-400 font-bold">{accessType}</span>
                                </p>
                            </div>
                            <div className="text-right hidden sm:block">
                                <span className="text-4xl font-bold text-white block">{files.length}</span>
                                <span className="text-sm text-slate-500 uppercase tracking-wider">Total Documents</span>
                            </div>
                        </div>
                        ...
                        {filteredFiles.length === 0 ? (
                            <div className="text-center py-20">
                                ...
                                <p className="text-slate-500">No matching files found.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                {filteredFiles.map((file, i) => (
                                    <motion.div key={file.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="group bg-white/5 hover:bg-white/10 border border-white/5 hover:border-violet-500/30 rounded-2xl p-5 cursor-pointer transition-all">
