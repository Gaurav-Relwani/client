...
interface FileItem {
  ...
  filename: string;
  owner: string;
  ownerName?: string; // <--- NEW: Server ab Name bhi bhejega
...
export default function Dashboard() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      // 1. UPLOAD
      await axios.post(`${API_URL}/upload`, { ...uploadData, department: currentDept }, { headers: { Authorization: `Bearer ${token}` } });
      showToast("Secure Upload Complete", 'success');
      closeModal();

      // 2. SILENT REFRESH (Errors ignored here so user sees Success)
      try {
        const res = await axios.post(`${API_URL}/department-data`, { department: currentDept, password: deptPassword }, { headers: { Authorization: `Bearer ${token}` } });
        setFiles(res.data.files);
        fetchStats();
      } catch (refreshErr) { console.log("Silent refresh skipped"); }

    } catch (err) { 
        showToast("Upload Rejected: Invalid Password", 'error'); 
    }
    finally { setLoading(false); }
  };
                        ...
                        <div className="p-8 relative z-10">
                            ...
                            {modals.delete && (
                                <div className="text-center space-y-4">
                                    ...
                                    <p className="text-slate-400 text-sm">Permanently purge <span className="text-white font-mono bg-white/10 px-1 rounded">{targetFile?.name}</span>? This is irreversible.</p>
                            ...
                            {/* AUTH/REQUEST/UPLOAD (Classic Modals) */}
